...
package Curse::Content::MultiPart;

use strict;
use warnings;

use base 'Curse::Content';
use bytes;
...
sub _parse_multipart_boundary {
    ...
    if (index($self->buffer->{buffer}, "\x0d\x0a--$boundary\x0d\x0a") == 0) {
        ...
        push @{$self->parts}, Curse::Content->new(
...
Curse::Content::MultiPart - HTTP MultiPart Content

=head1 SYNOPSIS

    use Curse::Content::MultiPart;

    my $content = Curse::Content::MultiPart->new;
    $content->parse('Content-Type: multipart/mixed; boundary=---foobar');
    my $part = $content->parts->[4];

=head1 DESCRIPTION

L<Curse::Content::MultiPart> is a generic container for HTTP multipart
content.

=head1 ATTRIBUTES

L<Curse::Content::MultiPart> inherits all attributes from L<Curse::Content>
...
L<Curse::Content::MultiPart> inherits all methods from L<Curse::Content> and
