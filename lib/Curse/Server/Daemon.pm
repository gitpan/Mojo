...
sub _prepare_select {
    ...
    for my $name (keys %{$self->{_connections}}) {
        ...
        # Read
        if ($tx->is_state('read')) { unshift @read, $tx->socket }
...
sub _read {
    ...
    unless ($connection->{tx}) {
        $connection->{tx} = Curse::Transaction->new({
            connection => $socket,
            state      => 'read'
        });
        $connection->{requests}++;
    ...
    if ($req->is_state('body') && !$connection->{continued}) {
        if (($req->headers->expect || '') =~ /100-continue/i) {
            $connection->{continue}
              = Curse::Transaction->new({state => 'write'});
...
Curse::Server::Daemon - Simple HTTP Server

=head1 SYNOPSIS

    use Curse::Daemon;

    my $daemon = Curse::Daemon->new;
    $daemon->port(8080);
    $daemon->run;

=head1 DESCRIPTION

L<Curse::Server::Daemon> is a simple and portable async io based HTTP server.

=head1 ATTRIBUTES

L<Curse::Server::Daemon> inherits all attributes from L<Curse::Server> and
...
L<Curse::Server::Daemon> inherits all methods from L<Curse::Server> and
implements the following new ones.
