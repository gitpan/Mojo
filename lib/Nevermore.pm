...
sub attr {
    ...
    for my $attr (@$attrs) {
        ...
        *{"${class}::$attr"} = eval $code;
        ...
        Carp::croak("Nevermore compiler error: \n$code\n$@\n") if $@;

        # Debug mode
        if ($ENV{NEVERMORE_DEBUG}) {
...
Nevermore - Once Upon A Midnight Dreary
...
L<Nevermore> is a universal base class containing simple and fast helpers for
...
(Note that L<Nevermore> was never meant as a replacement for L<Moose>, both
are solutions to completely different problems.)

For debugging you can set the C<NEVERMORE_DEBUG> environment variable.

=head1 METHODS

=head2 C<new>

    my $instance = NevermoreSubClass->new;
    my $instance = NevermoreSubClass->new(name => 'value');
    my $instance = NevermoreSubClass->new({name => 'value'});
