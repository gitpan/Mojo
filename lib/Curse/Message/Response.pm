...
package Curse::Message::Response;
...
sub cookies {
    ...
    if (my $cookie2 = $self->headers->set_cookie2) {
        return Curse::Cookie::Response->parse($cookie2);
    }

    # Set-Cookie
    elsif (my $cookie = $self->headers->set_cookie) {
        return Curse::Cookie::Response->parse($cookie);
    }
...
sub fix_headers {
    ...
    $self->headers->date(Curse::Date->new->as_string)
...
L<Curse::Message::Response> is a generic container for HTTP responses.

=head1 ATTRIBUTES

L<Curse::Message::Response> inherits all attributes from L<Curse::Message>
...
L<Curse::Message::Response> inherits all methods from L<Curse::Message> and
