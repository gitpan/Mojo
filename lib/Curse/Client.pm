    ...
    if (@$read) {
        ...
        if ($tx->is_state('read_continue')) {
            ...
            if ($res->is_state('done') && ($res->code == 100)) {
                $tx->res(Curse::Message::Response->new);
                $tx->{_continue} = 0;
            }
        }
...
L<Curse::Client> is a full featured async io HTTP 1.1 client.
...
L<Curse::Client> inherits all methods from L<Nevermore> and implements the
following new ones.
