...
package Curse::Server::Daemon::Prefork;

use strict;
use warnings;

use base 'Curse::Server::Daemon';
...
__PACKAGE__->attr('pid_file',
    chained => 1,
    default => sub {
        return catfile(splitdir(tmpdir()), 'curse_daemon_prefork.pid');
    }
);
...
sub run {
    ...
    $self->log('Parent started') if $self->debug;
...
sub _manage_children {
    ...
    # Too many children
        ...
        for my $idle (@idle) {
            ...
            kill 'HUP', $idle;
            $self->log("Child $idle stopped") if $self->debug;
...
sub _reap_child {
    ...
    while ((my $child = waitpid(-1, WNOHANG)) > 0) {
        $self->log("Child $child died") if $self->debug;
...
sub _spawn_child {
    ...
    unless ($child) {
        ...
        $self->log('Child started') if $self->debug;
...
=head1 SYNOPSIS
    ...
    use Curse::Daemon::Prefork;

    my $daemon = Curse::Daemon::Prefork->new;
    $daemon->port(8080);
    $daemon->run;

=head1 DESCRIPTION

L<Curse::Daemon::Prefork> is a simple prefork HTTP server.

=head1 ATTRIBUTES

L<Curse::Server::Daemon::Prefork> inherits all attributes from
L<Curse::Server::Daemon> and implements the following new ones.
...
=head2 C<debug>
    ...
    my $debug = $daemon->debug;
    $daemon   = $daemon->debug(1);
...
=head2 C<pid_file>
    ...
    $daemon      = $daemon->pid_file('/tmp/curse_daemon_prefork.pid');
...
L<Curse::Server::Daemon::Prefork> inherits all methods from
L<Curse::Server::Daemon> and implements the following new ones.
