...
__PACKAGE__->attr('url', chained => 1, default => sub { Curse::URL->new });
...
sub cookies {
    ...
    if (my $cookie = $self->headers->cookie) {
        return Curse::Cookie::Request->parse($cookie);
    }
...
sub fix_headers {
    ...
    if ((my $proxy = $self->proxy) && !$self->headers->proxy_authorization) {
        ...
        if (my $userinfo = $proxy->userinfo) {
            my $auth = Curse::ByteStream->new("$userinfo")->b64_encode;
...
sub proxy {
    ...
    # New proxy
    if ($url) {
        $url = Curse::URL->new($url) unless ref $url;
        $self->{proxy} = $url;
        return $self;
    }
    ...
    if ($url = $ENV{HTTP_PROXY}) {
        $self->{proxy} = Curse::URL->new($url) unless $self->{proxy};
    }
...
L<Curse::Message::Request> is a generic container for HTTP requests.

=head1 ATTRIBUTES

L<Curse::Message::Request> inherits all attributes from L<Curse::Message> and
...
L<Curse::Message::Request> inherits all methods from L<Curse::Message> and
implements the following new ones.
