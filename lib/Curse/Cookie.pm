...
sub expires {
    my ($self, $expires) = @_;
    if ($expires) {
        $self->{expires} = Curse::Date->parse($expires) unless ref $expires;
    }
    return $self->{expires};
}
...
sub max_age {
    my ($self, $max_age) = @_;
    if ($max_age) {
        $self->{max_age} = Curse::Date->parse("$max_age");
    }
    ...
    while (length $string) {
        ...
        //x) {
            ...
            if ($string =~ s/
                ...
                (!:\\(!:\\\")?)*   # Value
                \")                # Quote
            //x) { $value = Curse::ByteStream->new($1)->unquote }
...
Curse::Cookie - Cookie Base Class
...
L<Curse::Cookie> is a generic cookie base class.
...
L<Curse::Cookie> inherits all methods from L<Nevermore> and implements the
following new ones.
